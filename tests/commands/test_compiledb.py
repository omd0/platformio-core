...
"""
    ...
    result = proc.exec_command(
        ["platformio", "run", "-t", "compiledb"], cwd=project_dir
    )
    ...
    assert (
        len(compile_commands) > 0
    ), "compile_commands.json should contain at least one entry"
    ...
    for unit in compile_commands:
        ...
        assert (
            "command" in unit or "arguments" in unit
        ), "Each compilation unit must have 'command' or 'arguments'"

        # Check that compiler path is absolute
        if "command" in unit:
            command = unit["command"]
            assert command.startswith("/") or command.startswith(
                '"'
            ), f"Compiler path should be absolute: {command}"
            ...
            assert compiler_path.startswith("/") or compiler_path.startswith(
                '"'
            ), f"Compiler path should be absolute: {compiler_path}"
        ...
        if "command" in unit:
            ...
            assert (
                len(include_flags) > 0
            ), "Arguments should include -I flags for include paths"
...
"""
    ...
    for unit in compile_commands:
        if "command" in unit:
            command = unit["command"]
            assert (
                f"-I{custom_include}" in command or f'-I"{custom_include}"' in command
            ), f"Custom include path not found: {command}"
            assert (
                f"-I{another_include}" in command or f'-I"{another_include}"' in command
            ), f"Another include path not found: {command}"
        elif "arguments" in unit:
            arguments = unit["arguments"]
            ...
            assert (
                len(custom_includes) > 0
            ), f"Custom include path not found in arguments: {arguments}"
            assert (
                len(another_includes) > 0
            ), f"Another include path not found in arguments: {arguments}"
...
"""
    ...
    for unit in compile_commands:
        ...
        assert os.path.isabs(
            unit["file"]
        ), f"File path should be absolute: {unit['file']}"
        ...
        if "command" in unit:
            ...
            for include_path in include_matches:
                ...
                assert os.path.isabs(
                    include_path
                ), f"Include path should be absolute: {include_path}"
        elif "arguments" in unit:
            arguments = unit["arguments"]
            include_flags = [arg for arg in arguments if arg.startswith("-I")]
...
board = esp32dev
build_flags = -mlongcalls -fstrict-volatile-bitfields -fno-tree-switch-conversion -mfix-esp32-psram-cache-issue
"""
    with open(os.path.join(project_dir, "platformio.ini"), "w") as f:
        f.write(ini_content)
    ...
    for unit in compile_commands:
        if "command" in unit:
            ...
            for flag in gcc_only_flags:
                assert (
                    flag not in command
                ), f"GCC-specific flag '{flag}' should be filtered out: {command}"
        elif "arguments" in unit:
            arguments = unit["arguments"]
            for flag in gcc_only_flags:
                assert (
                    flag not in arguments
                ), f"GCC-specific flag '{flag}' should be filtered out: {arguments}"


def test_compiledb_esp32_definitions(tmpdir):
    """Test that ESP32-specific definitions are added to compile_commands.json"""

    # Create a test project with ESP32 platform
    project_dir = tmpdir.mkdir("test_esp32_definitions")

    # Create platformio.ini with ESP32 platform
    ini_content = """
[env:esp32]
platform = espressif32
board = esp32dev
"""
    ...
    # Run compiledb command using the tox environment platformio
    # Get the platformio executable from the tox environment
    tox_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), ".tox", "py310")
    platformio_exe = os.path.join(tox_dir, "bin", "platformio")
    
    if not os.path.exists(platformio_exe):
        # Fallback to system platformio if tox environment not available
        platformio_exe = "platformio"
    
    from platformio import proc

    result = proc.exec_command(
        [platformio_exe, "run", "-t", "compiledb"], cwd=project_dir
    )
    ...
    # Check that ESP32-specific definitions are present
    esp32_definitions = ["-D__XTENSA__", "-DARDUINO_ARCH_ESP32", "-DESP32"]

    for unit in compile_commands:
        if "command" in unit:
            command = unit["command"]
            for definition in esp32_definitions:
                assert (
                    definition in command
                ), f"ESP32 definition '{definition}' should be present: {command}"
        elif "arguments" in unit:
            arguments = unit["arguments"]
            for definition in esp32_definitions:
                assert (
                    definition in arguments
                ), f"ESP32 definition '{definition}' should be present: {arguments}"


def test_compiledb_compiler_path_resolution(tmpdir):
    """Test that compiler paths are properly resolved to absolute paths"""

    # Create a test project
    project_dir = tmpdir.mkdir("test_compiler_path_resolution")
...
"""
    ...
    # Check that compiler paths are absolute
    for unit in compile_commands:
        if "command" in unit:
            command_parts = unit["command"].split()
            compiler_path = command_parts[0].strip('"')
